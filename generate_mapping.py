...
def clean_text(text):
    ...
    # catches the same instruction word when nothing wraps it.
    cleaned = re.sub(r'\s+(terminate|continue|anchor|rotate|randomize)\.?\s*$', '', cleaned, flags=re.IGNORECASE)
...
PARA_ROUTING_RE = re.compile(
    ...
    r'rotate\b|randomize\b|anchor\b)',
    re.IGNORECASE
)


def looks_like_option_candidate(text):
    ...
    text itself -- see maybe_update_question_text), whole-line routing instructions, and
    anything implausibly long for a single answer option."""
    text = text.strip()
        ...
        if isinstance(it, Paragraph):
            ...
            if text:
                ...
                if stem:
                    ...
                    if looks_like_option_candidate(text):
...
def compute_family_reserve(parsed_all, family_rules, questions, other_positions, sav_value_labels):
...
def build_workbook(sav_path, qnr_path):
    ...
    # variable label (first family member that has a usable one, so every sibling
    # still shares identical text before " : "), and only fall back to the
    # questionnaire's text when the .sav genuinely has nothing there.
    base_text = {}
    for p in parsed_all:
        stem = p['stem']
        if base_text.get(stem):
            continue
